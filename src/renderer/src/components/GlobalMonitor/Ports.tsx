...
function Ports() {
    ...
    return (
        <div className="flex flex-col gap-2">
            <div>
                <div className="text-secondary-foreground">TCP ports in use</div>
                <ScrollArea className=" whitespace-nowrap rounded-md border">
                    <div className="flex p-1 mb-2">
                        {tcpProcesses ? (
                            tcpProcesses.map((process, i) => (
                                <PortStrip
                                    key={process.process}
                                    name={process.process}
                                    ports={process.byPort}
                                    color={selectColor(i)}
                                />
                            ))
                        ) : (
            ...
            <div>
                <div className="text-secondary-foreground">UDP ports in use</div>
                <ScrollArea className=" whitespace-nowrap rounded-md border">
                    <div className="flex p-1 mb-2">
                        {udpProcesses ? (
                            udpProcesses.map((process, i) => (
                                <PortStrip
                                    key={process.process}
                                    name={process.process}
                                    ports={process.byPort}
